...
/// with '-'.
pub(super) fn strict_slugify(s: &str) -> String {
  ...
  for c in iter {
    push_ascii_char(&mut slug, c as u8, &mut prev_is_dash);
  }
...
fn push_ascii_char(slug: &mut String, x: u8, prev_is_dash: &mut bool) {
  match x {
    b'a'..=b'z' | b'0'..=b'9' => {
      *prev_is_dash = false;
      slug.push(x.into());
